---
slug: "manual/bluetape4k-javers/0.3/getting-started"
...
`ensureSchema()` creates `javers_commit` and `javers_snapshot`. This convenience is useful for a local run; production schema changes remain the deployment owner's responsibility. Every repository operation opens an Exposed transaction, so an application-domain transaction and the JaVers write are not automatically one atomic unit. See the [Exposed guide](/manual/bluetape4k-javers/0.3/persistence/exposed/) before treating the snippet as production wiring.
...
Next, read [the audit model](/manual/bluetape4k-javers/0.3/architecture/audit-model/), then choose a persistence role in [the selection guide](/manual/bluetape4k-javers/0.3/persistence/selection-guide/).
