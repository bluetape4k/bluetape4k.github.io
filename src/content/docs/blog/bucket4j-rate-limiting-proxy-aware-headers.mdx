...
That is the useful boundary for the workshop. The HTTP layer does not need to know Bucket4j probe internals. It only
...
The permit decision is `result.isConsumed`. If one token was consumed, the request reaches the handler. If
