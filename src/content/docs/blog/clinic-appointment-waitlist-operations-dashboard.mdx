...
<div class="bt4k-operations-screen" data-visual-title="STAFF operations screen mockup">
  <img class="bt4k-screenshot" src="/assets/clinic-appointment-waitlist-operations-screen-dashboard-en.png" alt="A STAFF operations screen with waiting, in-progress, near-expiry, and review-needed metrics above an action queue, selected evidence, permitted work, and a terminal outcome decision" loading="lazy" />
  <p class="bt4k-operations-screen-caption">This is a screen design mockup, not a production capture. The values are illustrative; the reading order is the point: clinic metrics and readiness first, then the action queue, selected evidence, permitted work, and the terminal outcome decision.</p>
</div>
...
The previous article explained waitlist state and ownership. This article set the boundary for the screen that staff use to read and act on that state. The next article will connect [each command to its API request and re-read flow](/blog/clinic-appointment-waitlist-operations-command/) in smaller units.
