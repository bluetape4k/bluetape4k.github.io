...
<figure class="bt4k-architecture" data-diagram-title="Service text-processing boundaries and quality evidence">
  <img src="/assets/bluetape4k-text-part1-pipeline-01-en.png" alt="HTTP body limits lead to service input validation, normalization and routing, text analysis, service decisions, and quality gates" loading="lazy" />
  <figcaption>Limit the body at the transport boundary, validate the materialized string, and then attach only the analysis modules the service decision needs.</figcaption>
</figure>

Suppose a service needs to accept user text, prepare search/index terms, or filter abuse words. First cap the HTTP request body
before it becomes a `String`. Once materialized, validate blank input and character count. Then normalize the text and detect its
language when routing depends on it. Use tokenizers for Korean or Japanese text when search-friendly terms matter. Use
`text-search` when a keyword dictionary needs to be matched in one pass.
...
so the service should validate an already materialized string before processor work starts.
...
This validation does not cap allocation while an HTTP body is being decoded into a `String`; that limit belongs at the transport
boundary. The request models and service handler then provide a second guard for character count and blank input.
...
| API input defense | transport body limits, `tokenizer-core` request models, and safety examples |
