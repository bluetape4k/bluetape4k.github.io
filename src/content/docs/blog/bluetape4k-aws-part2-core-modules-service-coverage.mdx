...
blog:
  ...
  cardDescription: "Java SDK v2 path, Kotlin SDK path, CRT-backed S3 TransferManager, and the compileOnly dependency model."
---
...
<figure class="bt4k-architecture">
  ...
  <figcaption>The two SDK paths differ, but service code usually wants a small and consistent interface.</figcaption>
</figure>
...
a service needs OkHttp explicitly. So the Kotlin SDK path also has a CRT-first default with a clear
escape hatch.
...
| Service | Java SDK v2 path | Kotlin SDK path | Convenience features |
|---|---|---|---|
...
Use `bluetape4k-aws-java` when the service needs the broad AWS Java SDK v2 path or wants the S3
...
Use `bluetape4k-aws-kotlin` when new code wants to stay on the AWS Kotlin SDK path and keep native
...
SDK paths. The Java path keeps AWS Java SDK v2 coverage, adds coroutine bridges, and makes the S3
CRT/TransferManager/Virtual Threads route easier to adopt. The Kotlin path keeps the AWS Kotlin
...
SigV4 plugins, and runnable examples.
