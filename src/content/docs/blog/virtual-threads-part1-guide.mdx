...
thread-per-request 스타일을 훨씬 낮은 비용으로 다시 쓸 수 있게 만든 기술이다.

![Virtual Threads guide map](/assets/virtual-threads-part1-guide-01.png)

## Virtual Threads가 해결하는 문제

전통적인 servlet/JDBC 코드는 참 읽기 쉽다. 위에서 아래로 내려가고, 중간에 갑자기 callback 미로가
열리지도 않는다.
...
기다리는 동안 OS thread도 같이 묶인다. 요청 수가 많아지면 application은 CPU보다 thread 수에 먼저 막힌다.
...
이 구조를 그림으로 보면 더 단순하다. task는 virtual thread로 분리하고, 제한은 thread가 아니라 뒤쪽
resource에 건다.

중요한 단서가 있다. Virtual Thread는 "더 빠른 thread"가 아니다. CPU-bound 작업을 더 빨리 계산하지
않는다. 효과가 나는 곳은 많은 작업이 대부분의 시간을 기다리는 I/O-heavy service다.
...
바꿨더니 성능이 좋아졌다"가 아니라 "Virtual Threads로 바꿨더니 DB가 먼저 한계에 닿았다"가 된다.
...
code를 최소한의 변경으로 바로 쓸 수 있다"에 가깝다. 대신 그만큼 기본기는 더 중요해진다. resource limit,
