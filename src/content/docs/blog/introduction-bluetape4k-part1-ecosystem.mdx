...
Bluetape4k is a Kotlin/JVM library ecosystem for backend services. It does not try to wrap the
whole application as one large framework. Instead, it gives Spring Boot 4 and Ktor 3 services a set
of layer-specific modules they can add where they need them.

This first article draws the map. Later parts will go into repository architecture, API examples,
and adoption paths.
...
A practical way to understand Bluetape4k is to split it into five layers. The table below gives the
overview; the following sections explain what each layer contributes.
...
Spring Boot 4 or Ktor 3 owns the application boundary. Bluetape4k fills the domain, data,
infrastructure, and foundation layers underneath it.

## Application Layer

### Spring Boot 4

Purpose: let Spring Boot services assemble Bluetape4k data, cache, infrastructure, and testing
modules without hand-wiring the same setup repeatedly.
...
Purpose: let Ktor services use cloud, coordination, and graph capabilities around suspend APIs and
coroutine clients.
...
Purpose: prevent duplicate execution of batch jobs, schedulers, polling workers, cache warmers, and
migration gates in distributed services.
...
Purpose: let services choose between ordinary JVM image processing and high-throughput libvips
processing inside the same ecosystem.
...
Purpose: make Korean and Japanese tokenization, language detection, and keyword search reusable in
Kotlin services.
...
Purpose: provide a Kotlin DSL-oriented persistence model and put JDBC and R2DBC behind a consistent
repository pattern.
...
Purpose: reduce vendor API differences across graph databases and give graph-heavy domains a common
abstraction.
...
Purpose: provide data-access utilities that do not belong specifically to the Exposed or GraphDB
repositories.
...
Purpose: make AWS Java SDK v2 and AWS Kotlin SDK fit the execution model of Kotlin services.
...
Purpose: provide the messaging base for event-driven services and asynchronous integration.
...
Purpose: let services handle local cache, distributed cache, and Redis client integration with the
...
Purpose: connect metrics, tracing, resilience, and rate limiting with the rest of the ecosystem.
...
Purpose: let services choose Kotlin coroutines or Java Virtual Threads based on the runtime shape
they need.
...
Purpose: reduce repeated test setup and make integration tests easier to assemble.
...
Purpose: keep compatible dependency sets available even when repositories evolve on separate
release cadences.
...
In a real service, Spring Boot 4 or Ktor 3 forms the application boundary. Teams then add
Foundation, Data, Infrastructure, and Domain Capability modules underneath that boundary as needed.
...
The usual path starts with the Foundation layer: core utilities, coroutine helpers, testing
support, and BOM alignment. From there, the service adds Data and Infrastructure modules. Domain
capabilities such as leader election, audit trails, image processing, and text processing come in
when the product needs them.

The key point is that Bluetape4k is not one large framework. It is an ecosystem of small modules
that existing Spring Boot 4 or Ktor 3 services can adopt gradually.

The next articles will break down each layer further and look at representative repositories with
architecture diagrams and concrete examples.
