...
sidebar:
  order: -202605291101
blog:
  date: 2026-05-29T11:23:58+09:00
...
Part 1 covered the big picture. This post is about rules that show up immediately in code. The
...
<figure class="bt4k-architecture">
  <img src="/assets/virtual-threads-part2-rules-01-en.png" alt="Practical virtual thread rules from bluetape4k-workshop" loading="lazy" />
...
flow still reads in a straight line.
...
That uses virtual threads by name, but the mindset is still pooling. The default should be a
...
connections, and external API quotas are not infinite. A semaphore makes the protected resource
explicit.
...
for every thread creation style. Check whether the propagation boundary is structured.
