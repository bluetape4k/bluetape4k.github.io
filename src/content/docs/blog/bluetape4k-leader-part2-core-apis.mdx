...
The return type is `T?`. When the caller gets `null`, it means the lock was contended and the action was
not run. This mirrors the ShedLock skip-on-fail semantics: contention is not an error, it is the expected
...
released until the returned future completes, preventing the action from outliving the lease.
...
lock is released. A cancelled leader holding work is not a graceful shutdown — it is an interrupted one.
The action must not swallow cancellation.
...
Constraints on the tenant id and lock name:
...
The critical contract: **state is a best-effort snapshot**. Do not use it to decide whether to run an
...
- Issue: [#40 Blog Part 2: LeaderElector core APIs and execution models](https://github.com/bluetape4k/bluetape4k.github.io/issues/40)
