...
<figure class="bt4k-sequence">
  <img src="/assets/bluetape4k-leader-runifleader-sequence-01-en.png" alt="runIfLeader success and skip sequence diagram" loading="lazy" data-diagram-title="runIfLeader execution and skip flow" />
...
| `leaseTime` | 60s | Lease TTL for holding leadership |
...
`waitTime` expires, `runIfLeader` returns `null`. `leaseTime` is the TTL of each leadership lease. If a
node dies without releasing the lock, lease expiry eventually allows another node to acquire it. With
`autoExtend = true`, supported single-leader backends renew that lease while the action is still running.

**`minLeaseTime`** — guarantees the lock is held for at least N seconds even when the action completes
in milliseconds. Local electors wait before releasing; supported distributed backends delegate the
remaining minimum lease to storage TTL so the caller can return immediately.
