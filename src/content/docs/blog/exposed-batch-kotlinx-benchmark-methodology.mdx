...
<p class="bt4k-post-meta">2026-06-28 · bluetape4k-exposed batch benchmark</p>
...
<figure class="bt4k-architecture" data-diagram-title="Batch benchmark report generation path">
...
end-to-end profiles. Here, `ops/sec` counts completed batch-job invocations, not processed rows;
each invocation processes the configured `dataSize`. Higher `ops/sec` is better, while `avg ms`
is the mean duration of one full job and is lower-is-better.
...
<figure class="bt4k-chart" data-diagram-title="Seed job throughput by database">
...
<figure class="bt4k-chart" data-diagram-title="PostgreSQL end-to-end job throughput by partition count">
...
- [`utils/batch/benchmark/README.md`](https://github.com/bluetape4k/bluetape4k-exposed/blob/1.11.0/utils/batch/benchmark/README.md)
