---
slug: "ko/manual/bluetape4k-image/0.4/integrations/ktor"
...
[Ktor 이미지 API 워크숍](/ko/manual/bluetape4k-image/0.4/modules/ktor-image-api/)은 두 라우트 계열과 테스트를 함께 보여 준다. 가장 작은 전체 구조로 실행한 뒤 예제 구성 요소를 애플리케이션 저장소, 인증, 요청률 제한과 관측 코드로 바꾼다.

OCR은 라우트 모듈에 숨겨져 있지 않다. [Ktor OCR 워크숍](/ko/manual/bluetape4k-image/0.4/modules/ktor-ocr-api/)에서 멀티파트 입력, 프레임워크와 무관한 OCR 엔진, 응답 모델과 실패 처리를 명시적으로 조합한다.
