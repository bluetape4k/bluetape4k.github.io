---
slug: "ko/manual/bluetape4k-image/0.3/guides/backend-selection"
...
![Scrimage, Java 21 JVips, Java 25 FFM 백엔드 선택 지도](/manual-assets/bluetape4k-image/0.3/backends/backend-decision-map.svg)
...
불변 이미지 도우미, Java2D 그리기, 필터 DSL, 유사도 알고리즘, OCR 입력이 필요하거나 네이티브 패키지를 설치할 수 없다면 Scrimage 경로를 쓴다. 첫 구현을 만들기도 가장 쉽다. [기본 처리 워크숍](/ko/manual/bluetape4k-image/0.3/modules/basic-processing/)도 이 경로를 사용한다.
...
JDK 21을 유지해야 하면 [Java 21 JNI](/ko/manual/bluetape4k-image/0.3/native/java21-jni/)를 고른다. JDK 25와 <code>--enable-native-access=ALL-UNNAMED</code>을 배포 계약에 넣을 수 있을 때만 [Java 25 FFM](/ko/manual/bluetape4k-image/0.3/native/java25-ffm/)을 쓴다.

## 백엔드 둘을 무심코 배포하지 않기

편하다는 이유로 네이티브 구현 두 개를 일반 런타임 클래스패스에 함께 넣지 않는다. 컴파일에는 공통 [Vips API](/ko/manual/bluetape4k-image/0.3/native/vips-api/)를 쓰고 구현은 하나만 명시적으로 선택한다. 마이그레이션 검증이나 벤치마크에서는 둘을 비교할 수 있지만 운영 소유권은 하나로 정해야 한다.

## 측정해서 결정하기

저장소 벤치마크는 방향을 잡는 자료로 쓰고, 실제 서비스의 대표 이미지, 동시 실행 수, 파일과 네트워크 경계, 메모리를 다시 측정한다. [성능 선택](/ko/manual/bluetape4k-image/0.3/guides/performance-selection/)에 판단 절차를 정리했다.
