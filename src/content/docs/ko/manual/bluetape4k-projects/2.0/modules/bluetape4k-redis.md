---
slug: "ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis"
...
manual:
  ...
  sourcePath: "docs/manual/bluetape4k-projects/ko/modules/bluetape4k-redis.md"
  minorVersion: "2.0"
  releaseRef: "2.0.0"
...
이 좌표 하나로 두 하위 artifact가 compile/runtime classpath에 들어옵니다. 하나만 사용한다면 [선택 의존성으로 전환](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/selective-dependency-migration/)해 classpath와 운영 책임을 줄입니다.
...
새 코드가 Lettuce만 사용하고 기존 Redisson 호출을 모두 제거했다면 다음 단계에서 좌표를 `bluetape4k-lettuce`로 바꿉니다. 우산 모듈에는 호출할 factory나 초기화 함수가 없으므로 client 생성 예제는 [Lettuce 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-lettuce/)과 [Redisson 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redisson/)에서 확인합니다.
...
1. [우산 의존성 계약](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/umbrella-dependency-contract/) — `api`로 내보내는 두 artifact와 우산 모듈이 하지 않는 일을 확인합니다.
2. [Lettuce와 Redisson 선택](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/lettuce-vs-redisson/) — command 중심 client와 분산 객체 중심 client를 요구사항으로 비교합니다.
3. [선택 의존성으로 전환](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/selective-dependency-migration/) — 사용처를 조사하고 한 client 좌표로 줄이는 순서를 설명합니다.
4. [Lifecycle과 Codec 경계](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/lifecycle-codec-boundaries/) — client 종료, coroutine, keyspace와 wire format을 구현별로 관리합니다.
5. [Cache provider와 Spring Data Redis 분리](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/cache-spring-separation/) — client helper, cache abstraction, serializer 모듈을 구분합니다.
6. [테스트·운영·생태계 경로](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redis/testing-operations-ecosystem/) — 하위 모듈 테스트와 운영 지표에서 workshop·Exposed cache로 이어갑니다.
...
- [`infra/redis/README.ko.md`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/redis/README.ko.md)
- [`bluetape4k-lettuce` 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-lettuce/)
- [`bluetape4k-redisson` 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-redisson/)
- [`bluetape4k-cache-lettuce` 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-cache-lettuce/)
- [`bluetape4k-cache-redisson` 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-cache-redisson/)
- [`bluetape4k-spring-boot-redis` 매뉴얼](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-spring-boot-redis/)
