---
slug: "ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka"
...
manual:
  ...
  sourcePath: "docs/manual/bluetape4k-projects/ko/modules/bluetape4k-kafka.md"
  minorVersion: "2.0"
  releaseRef: "2.0.0"
...
Kafka 4.x line은 [`bluetape4k-kafka4`](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka4/)입니다. 두 artifact는 같은 package와 class 이름을 공유하므로 한 애플리케이션에 함께 넣지 않습니다.
...
1. [모듈 경계와 client·record helper](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/module-boundary-client-records/)에서 Kafka 3.x line, resource ownership과 작은 factory를 확인합니다.
2. [Codec, wire format과 보안](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/codecs-wire-format-security/)에서 type header allowlist, poison pill과 선택형 binary backend를 결정합니다.
3. [Coroutine producer](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/coroutine-producer/)에서 callback, cancellation, `Flow` concurrency와 `flush` 시점을 익힙니다.
4. [Spring Kafka template과 listener adapter](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/spring-kafka-templates-listeners/)에서 두 `suspendSend` 계열과 Reactor Kafka template의 차이를 구분합니다.
5. [Kafka Streams DSL factory](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/kafka-streams-dsl/)에서 serde, state store와 topology ownership을 확인합니다.
6. [실패, 테스트, 운영과 생태계](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/failures-testing-operations-ecosystem/)에서 delivery, offset, 지표, `kafka4`와 `kafka-logback` 경계를 정리합니다.
...
| 작업 | 시작 API | 다음 장 |
| --- | --- | --- |
| native producer·consumer 생성 | `producerOf`, `consumerOf` | [모듈 경계](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/module-boundary-client-records/) |
| metric·topic-partition helper | `getMetricValueOrNull`, `topicPartitionOf` | [모듈 경계](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/module-boundary-client-records/) |
| 문자열·byte array·JSON·binary codec | `KafkaCodec`, `KafkaCodecs` | [Codec과 보안](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/codecs-wire-format-security/) |
| native producer를 suspend/Flow로 사용 | `suspendSend`, `sendAsFlow`, `sendAndForget` | [Coroutine producer](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/coroutine-producer/) |
| Spring `KafkaOperations` 발송 | `io.bluetape4k.kafka.spring.suspendSend` | [Spring 연동](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/spring-kafka-templates-listeners/) |
| Reactor Kafka producer·consumer | `SuspendKafkaProducerTemplate`, `SuspendKafkaConsumerTemplate` | [Spring 연동](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/spring-kafka-templates-listeners/) |
| Streams parameter 구성 | `consumedOf`, `materializedOf`, `streamJoinedOf` 등 | [Streams DSL](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka/kafka-streams-dsl/) |
...
`bluetape4k-kafka`는 Kafka 3.9.x/Spring Kafka 3.x/Jackson 2 line입니다. Kafka 4.2.x/Spring Kafka 4.x/Jackson 3 line은 `bluetape4k-kafka4`를 선택합니다. Logback event를 Kafka로 내보내는 기능은 별도 [`bluetape4k-kafka-logback`](/ko/manual/bluetape4k-projects/2.0/modules/bluetape4k-kafka-logback/) artifact가 맡습니다.
...
- [모듈 README](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/README.ko.md)
- [2.0.0 build](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/build.gradle.kts)
- [`ProducerCoroutines.kt`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/src/main/kotlin/io/bluetape4k/kafka/coroutines/ProducerCoroutines.kt)
- [`KafkaCodec.kt`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/src/main/kotlin/io/bluetape4k/kafka/codec/KafkaCodec.kt)
- [`SuspendKafkaProducerTemplate.kt`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/src/main/kotlin/io/bluetape4k/kafka/spring/core/SuspendKafkaProducerTemplate.kt)
- [`SuspendKafkaConsumerTemplate.kt`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/src/main/kotlin/io/bluetape4k/kafka/spring/core/SuspendKafkaConsumerTemplate.kt)
- [`KStreamDslTest.kt`](https://github.com/bluetape4k/bluetape4k-projects/blob/2.0.0/infra/kafka/src/test/kotlin/io/bluetape4k/kafka/streams/kstream/KStreamDslTest.kt)
