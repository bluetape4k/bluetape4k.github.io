...
blog:
  ...
  cardDescription: "패키지의 구성 상품, 선택 규칙, 선후행 관계가 환자의 여러 방문과 실제 예약 항목으로 이어지는 경계를 설명합니다."
...
고객이 후보를 거부하거나 새 자원 점유에 실패했을 때도 기존 확정 예약을 먼저 지우지 않습니다. 새 제안 리비전과 자원 점유를 검증한 뒤 고객 동의가 완료될 때만 확정 제안을 교체합니다. 실패하면 기존 `confirmedProposalId`와 자원 점유를 그대로 유지합니다.
...
패키지 그래프가 만들어졌다고 환자 A의 달력에 방문이 자동으로 생기지는 않습니다. 다음 글 [상품 BOM은 어떻게 AppointmentPlan과 방문 계획으로 이어지는가](/ko/blog/clinic-appointment-execution-bom-to-appointment-plan/)에서는 구매 이벤트에 담긴 상품 BOM이 `AppointmentPlan` 리비전, `PlannedTreatment`, 한 번의 `Appointment`와 여러 `AppointmentItem`으로 이어지는 경로를 따라가겠습니다. 계획 생성과 방문 확정, 실제 임상 완료를 어디에서 분리하는지가 다음 질문입니다.
...
- [패키지 실행 그래프와 예약 설계](https://github.com/bluetape4k/clinic-appointment/blob/develop/docs/superpowers/specs/2026-07-29-issue-184-visit-commitment-design.md)
