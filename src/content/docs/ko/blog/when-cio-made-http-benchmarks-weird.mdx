...
description: io/http HTTP client 튜닝, Vert.x pool 병목 수정, 그리고 처음엔 실망스러웠던 Ktor CIO 숫자가 왜 유용한 단서였는지 정리한 benchmark 이야기.
sidebar:
  order: -202605290852
blog:
  ...
  cardDescription: Ktor CIO의 실망스러운 첫 성능에서 출발해 공정한 benchmark, Vert.x pool 병목 수정, workload별 HTTP client 선택 기준까지 정리한 이야기.
---
...
<figure class="bt4k-blog-hero">
  ...
  <figcaption>첫 이미지는 분위기를 잡는 용도다. benchmark chart는 이야기가 충분히 쌓인 뒤에 나온다.</figcaption>
</figure>
...
<figure class="bt4k-architecture">
  ...
  <figcaption>Ktor CIO의 실망스러운 첫 성능에서 출발해 공정한 benchmark, Vert.x pool 병목 수정, workload별 HTTP client 선택 기준까지 정리한 이야기.</figcaption>
</figure>
