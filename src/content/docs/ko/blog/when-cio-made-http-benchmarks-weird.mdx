...
description: io/http HTTP client benchmark에서 Ktor CIO의 느린 숫자가 어떻게 Vert.x pool 병목, fixture 문제, workload별 선택 기준을 드러냈는지 정리한 이야기.
sidebar:
  order: -202605290852
blog:
  ...
  cardDescription: Ktor CIO의 느린 첫 숫자에서 출발해 공정한 benchmark, Vert.x pool 병목 수정, workload별 HTTP client 선택 기준까지 정리한 이야기.
---
...
<figure class="bt4k-architecture">
  ...
  <figcaption>Ktor CIO의 느린 첫 숫자에서 출발해 공정한 benchmark, Vert.x pool 병목 수정, workload별 HTTP client 선택 기준까지 정리한 이야기.</figcaption>
</figure>
...
Benchmark는 "내 코드 빠르다"를 증명하는 도구만은 아니다. 가끔은 "방금 본 숫자를 그대로 믿으면
