...
description: 잘못된 가정에서 시작한 bluetape4k-projects 버그 수정 다섯 가지와, 어떻게 찾고 고치고 다시 막았는지 돌아본 글.
sidebar:
  order: -202605290925
blog:
  date: 2026-05-29T09:25:00+09:00
  image: /assets/embarrassing-bugs-hero.png
  imageAlt: 버그 리포트가 검증 코드와 guardrail로 바뀌는 소개용 일러스트
...
<figure class="bt4k-blog-hero">
  <img src="/assets/embarrassing-bugs-hero.png" alt="버그 리포트가 검증 코드와 guardrail로 바뀌는 소개용 일러스트" loading="eager" />
  <figcaption>민망한 실수로 시작한 수정도, 다음 실수를 조용히 막는 guard로 남으면 꽤 쓸모 있다.</figcaption>
</figure>
...
<figure class="bt4k-architecture">
  <img src="/assets/embarrassing-bug-fix-loop-01-ko.png" alt="Pastel flow chart showing mistake, signal, fix, and guard for embarrassing bug fixes" loading="lazy" />
...
그런데 `JavaPropsMapper`는 Properties 값을 untyped string으로 읽고, nullable numeric Kotlin constructor
...
재발 방지 장치는 간단하다. format-specific bug는 format-specific mapper에서만 고친다. 그리고
...
부끄러운 버그의 장점은, 고치고 나면 guard가 선명해진다는 점이다. 어려운 설계 논쟁이 아니라
