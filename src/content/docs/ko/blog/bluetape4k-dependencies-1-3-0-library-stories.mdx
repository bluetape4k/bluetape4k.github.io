---
title: "bluetape4k-dependencies 1.3.0 활용기 Part 1: 새 기능도 넣고 똥도 치우고"
...
`javers`의 버전도 같이 올라갑니다. 중요한 건 버전 숫자 자체가 아닙니다. 실제로 궁금한 건 이런 질문입니다.
...
여기서는 `dependencies` 내부의 catalog 관리 방식보다, 각 라이브러리에서 무엇이 좋아졌고 어떤 부분을 보강했는지
살펴봅니다. 코드와 운영 예시에서 바로 확인할 수 있는 변화에 초점을 맞춥니다.
...
cache 장애는 늦게 발견되면 데이터 일관성을 흔들 수 있습니다. 읽기는 계속 빠르게 성공하는 것처럼 끝나는데,
...
이 기능은 Ktor service에서 전역 logging 체계를 건드리지 않고 CloudWatch로 필요한 metric이나 log event만 보낼 때
도움이 됩니다. 다만 cloud 연동을 붙일 때는 늘 같은 질문을 해야 합니다.
...
물론 `Source`를 받는다고 모든 문제가 사라지지는 않습니다. Scrimage나 libvips가 decode/encode 단계에서 buffer를
잡을 수 있습니다. 그래서 문서에는 메모리 사용을 줄일 수 있는 지점과 backend 특성상 buffer가 생길 수 있는 지점을
같이 적어야 합니다.

image/OCR 기능을 붙인다면 option보다 먼저 파일 흐름을 점검하는 편이 좋습니다. 파일은 어디서 열리고, 어디서 닫히고,
...
불필요하게 커지지 않는지, 다음 기능선과 현재 안정화된 패치 라인을 섞어 이해하지 않는지를 보는 쪽이 더 중요합니다.

## 마무리

`dependencies 1.3.0`에서 바로 점검해 볼 만한 내용은 다음 정도입니다.
