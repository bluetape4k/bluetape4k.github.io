---
title: "bluetape4k-dependencies 1.3.0으로 서비스 조합하기"
...
<p class="bt4k-post-meta">2026-06-28 · bluetape4k dependencies 1.3.0 service composition</p>
...
이번 글은 `dependencies 1.3.0`을 가져온 뒤, 실제 서비스에서 어떤 모듈을 함께 고르면 좋은지 예제로 정리합니다.
여기서는 사용하는 쪽의 선택만 봅니다. 중요한 것은 "버전을 어디에 적느냐"가 아니라 "어떤 기능
경계에 어떤 모듈을 붙이느냐"입니다.
...
이 예제에서 `Leader`는 멋을 내려고 넣는 모듈이 아닙니다. worker가 세 대 떠 있는데 세 대가 같은 row를 집어가면,
그날의 배포는 기술 블로그가 아니라 반성문 소재가 됩니다. `Leader`를 쓰면 "이 작업은 지금 누가 대표로 실행하는가"를
코드 경계에 드러낼 수 있습니다.
...
이 값이 중요한 이유는 간단합니다. write-behind cache는 실패를 늦게 알수록 위험합니다. 겉으로는 요청이 성공한 것처럼
지나가는데, 뒤에서 flush가 멈추면 데이터 일관성이 깨질 수 있습니다. "평화로운 상태"가 좋은 징조가 아닐 때가 있습니다.
...
핵심은 OCR option보다 파일이 메모리에 올라오는 지점입니다. OCR 품질을 올리는 것도 중요하지만, 그 전에 서버가
파일 하나 때문에 숨이 차면 아무 소용이 없습니다.
...
이 질문에 답하면 필요한 모듈이 꽤 자연스럽게 정해집니다. 버전은 BOM에 맡기고, 사람은 서비스 경계를 보는 쪽이 덜
피곤합니다.
