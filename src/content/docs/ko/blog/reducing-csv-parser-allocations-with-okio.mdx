---
title: "Okio Segment로 CSV 파서 Allocation 줄이기"
description: bluetape4k-csv가 지원 가능한 UTF-8 CSV 파싱을 Okio segment 기반 fast path로 옮겨 CsvRecordReader 처리량을 약 2.3배 개선한 과정입니다.
sidebar:
  order: -202605290032
blog:
...
단순하고 정확했지만, 큰 CSV를 읽을 때 모든 문자를 `Reader`에서 decode한 뒤 `StringBuilder`에
누적하고 다시 field 문자열로 만드는 흐름이었다.
...
위에서 빠르게 하고, field decode는 field가 끝난 뒤 한 번만 수행한다.
