...
description: bluetape4k-csv가 지원되는 UTF-8 CSV 파싱을 Okio segment 기반 fast path로 옮겨 CsvRecordReader 처리량을 약 2.3배 개선한 과정입니다.
sidebar:
  order: -202605290032
blog:
  date: 2026-05-29T00:32:00+09:00
  image: /assets/csv-parser-okio-hero.png
  imageAlt: CSV parser가 불필요한 copy 없이 byte segment를 읽는 소개용 일러스트
  cardDescription: "CSV parser 최적화 작업을 Reader 기반 파싱에서 Okio UnsafeCursor segment scanning까지 benchmark와 함께 정리합니다."
  tags: ["kotlin","optimization","performance"]
---

<figure class="bt4k-blog-hero">
  <img src="/assets/csv-parser-okio-hero.png" alt="CSV parser가 불필요한 copy 없이 byte segment를 읽는 소개용 일러스트" loading="eager" />
  <figcaption>Byte-first parser는 특별한 알고리즘보다 먼저 불필요한 복사부터 줄인다.</figcaption>
</figure>

<p class="bt4k-post-meta">2026-05-29 · bluetape4k csv performance note</p>

`bluetape4k-projects`의 `bluetape4k-csv` 모듈은 원래 `Reader` 기반 CSV lexer를 사용했다. 구조는
단순하고 정확했지만, 큰 CSV를 읽을 때는 모든 문자를 `Reader`에서 decode한 뒤 `StringBuilder`에
...
<figure class="bt4k-architecture">
  <img src="/assets/csv-okio-throughput-comparison-01-ko.png" alt="CSV parser throughput comparison chart showing existing and Okio ops per second" loading="lazy" />
  <figcaption>CSV parser 최적화 작업을 Reader 기반 파싱에서 Okio UnsafeCursor segment scanning까지 benchmark와 함께 정리합니다.</figcaption>
</figure>
...
좋았어도 성능 개선폭이 작았다. large lexer 기준으로는 약 22% 개선에 그쳤다.
...
중요한 점은 cursor를 write 용도로 쓰지 않았다는 것이다. source buffer의 segment는 읽기만 하고,
...
특히 `extra_words.csv`처럼 실제 fixture에 가까운 큰 CSV 파일을 통째로 비교했다. 이 fixture는 세그먼트
...
UTF-8 파일 저장 경로라면 `BufferedSink` 기반 fast path를 둘 수 있다. 행 단위로 큰 `String`을
...
명확하다. 읽기에서는 `BufferedSource`와 `UnsafeCursor`가 allocation을 줄였고, 쓰기에서는
`BufferedSink`가 row pipeline의 중간 `String`과 작은 `Writer.write` 호출을 줄일 가능성이 있다.

## 참고 링크

- Follow-up article: [CSV Writer가 모래주머니를 벗었을 때](/ko/blog/csv-writer-okio-buffered-sink/)
