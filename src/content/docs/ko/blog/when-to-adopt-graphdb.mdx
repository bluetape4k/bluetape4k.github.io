...
description: bluetape4k benchmark 결과를 바탕으로 GraphDB 도입 판단, PostgreSQL recursive traversal, AGE, Neo4j, Memgraph, 그리고 GraphDB가 적합한 use case를 정리합니다.
sidebar:
  order: -202605281930
blog:
  ...
  cardDescription: GraphDB 도입 판단, PostgreSQL traversal baseline, AGE, Neo4j, Memgraph, 그리고 graph storage가 적합한 use case를 benchmark 결과로 정리합니다.
---
...
recursive SQL보다 읽기 쉽다. 하지만 도입 판단을 query 문법만 보고 하면 위험하다. GraphDB는 새 운영
표면, 새 query planner, 새 데이터 적재 방식, 새 index, 새 monitoring 기준, 그리고 종종 relational source
of truth 옆의 두 번째 저장 모델을 의미한다.
...
최근 bluetape4k graph benchmark 작업은 이 질문을 가정이 아니라 측정 결과로 확인하려는 시도였다.
...
<figure class="bt4k-architecture">
  ...
  <figcaption>GraphDB 도입 신호는 길고 선택적인 path-shaped traversal에서만 나왔습니다. PostgreSQL은 여러 bounded relational traversal에서 여전히 강했습니다.</figcaption>
</figure>

## 무엇을 비교했나

처음 아이디어는 abuser/fraud detection이었다. AGE + Exposed, Exposed, JPA를 비교하고 detection latency와
성공률을 보자는 방향이었다. 이 domain은 유용하지만, fraud detection은 benchmark를 잘못 설계하면 오해를
만들기 쉽다. 모든 가능한 path를 찾는 방식으로 만들면 GraphDB도 path explosion을 없애 주지 못한다.
...
이 workload는 GraphDB가 관심 가질 만한 성질을 가진다.
...
query shape가 약하면 쉽게 불공정해진다. 도입 판단에서는 PostgreSQL도 현실적인 baseline으로 봐야 한다.

## GraphDB 코드는 어떤 모습인가

bluetape4k에서는 application code를 domain에 가깝게 유지하면서 backend 구현이 TinkerGraph, Neo4j, Memgraph 같은 graph engine으로 operation을 mapping한다. social-network workshop은 `GraphOperations`로 vertex를 만들고, typed edge로 연결하고, bounded traversal operation을 실행한다.
...
이번 benchmark 판단은 GraphDB syntax만 보고 한 것이 아니다. 같은 authorization inheritance 질문을 native
...
JDBC recursive CTE는 PostgreSQL을 source of truth로 유지하면서 SQL이 membership path를 확장하게 한다.
...
JDBC iterative traversal은 recursive SQL을 쓰지 않고 frontier를 한 단계씩 확장한다. 각 step이 selective하고 index를 잘 타면 의외로 빠를 수 있다.
...
path workload가 충분히 길고 선택적일 때만 운영 비용을 상쇄한다.
...
이 결과는 중요하다. 단순한 이야기를 막기 때문이다. Domain이 graph 모양이라고 해서 곧바로 GraphDB가
필요한 것은 아니다. 데이터가 작거나, path depth가 짧거나, indexed relational traversal로 충분히 줄일 수
있는 query라면 PostgreSQL이 더 단순하고 더 빠를 수 있다.

AGE는 특히 좋은 반례였다. AGE는 PostgreSQL 안에서 Cypher syntax를 제공하지만, 이 benchmark에서는 속도
기반 도입 근거를 만들지 못했다. 편한 표현력과 좋은 실행 성능은 다르다.

## Path를 길게 잡으니 GraphDB 신호가 나왔다

다음 단계에서는 데이터 크기와 traversal 길이를 키웠다. adoption probe는 `large` fixture에서 다음 시나리오를 사용했다.
...
여기서 유의미한 신호가 처음 나왔다. `long-chain`에서 Neo4j는 PostgreSQL iterative traversal보다 3.74배,
PostgreSQL recursive CTE보다 4.35배 빨랐다. 이런 결과라면 GraphDB 추가 평가를 계속할 근거가 된다.
...
이 방향이 다음 benchmark로 이어갈 가치가 있다.
