...
blog:
  ...
  imageAlt: bluetape4k backend ecosystem을 연결된 building block으로 표현한 소개용 일러스트
...
<figure class="bt4k-blog-hero">
  <img src="/assets/bluetape4k-ecosystem-hero.png" alt="bluetape4k backend ecosystem을 연결된 building block으로 표현한 소개용 일러스트" loading="eager" />
  <figcaption>ecosystem은 먼저 책임들이 어떻게 연결되는지 보는 지도에서 시작한다.</figcaption>
</figure>

<p class="bt4k-post-meta">Part 1 · Ecosystem overview</p>

Bluetape4k는 Kotlin/JVM 기반 서비스를 만들 때 반복해서 필요한 기능을 모듈화한 라이브러리 생태계다.
하나의 프레임워크로 애플리케이션 전체를 감싸기보다, Spring Boot 4와 Ktor 3 서비스가 필요한 계층별
기능을 골라 가져가는 구조에 가깝다.

Part 1의 범위는 전체 지도다. 각 repository의 상세 architecture, API 예제, 적용 방식은 Part 2 이후에서
다룬다.
...
Bluetape4k는 크게 다섯 layer로 볼 수 있다. 표는 overview 용도이므로 짧게 유지하고, 각 layer의 구체적인
모듈과 역할은 아래 섹션에서 따로 설명한다.
...
이 구조에서 application layer는 Spring Boot 4 또는 Ktor 3가 담당한다. Bluetape4k는 그 아래에 있는
domain, data, infra, foundation layer를 채우는 모듈들을 제공한다.
...
개발 목적: coroutine-first service framework인 Ktor에서 cloud, coordination, graph 기능을 바로 사용할
수 있게 한다.
...
개발 목적: 일반적인 JVM 이미지 처리와 고성능 대량 이미지 처리를 같은 ecosystem 안에서 선택할 수 있게
한다.
...
개발 목적: JPA보다 Kotlin DSL에 가까운 persistence model을 제공하고, JDBC와 R2DBC를 같은 repository
pattern 안에서 다룬다.
...
개발 목적: 운영 환경에서 필요한 metrics, tracing, resilience, rate limiting 기능을 ecosystem 안에서
연결한다.
...
개발 목적: 여러 repository가 독립적으로 발전해도 사용자가 호환 가능한 dependency set을 가져갈 수 있게
한다.
...
실제 서비스에서는 Spring Boot 4 또는 Ktor 3가 application boundary가 되고, 그 아래에 Foundation, Data,
...
보통은 Foundation layer에서 core, coroutine, testing, BOM을 먼저 가져오고, 이후 Data layer와
...
module을 작은 단위로 선택하고, 이미 쓰고 있는 Spring Boot 4/Ktor 3 서비스에 점진적으로 얹는 ecosystem이다.
