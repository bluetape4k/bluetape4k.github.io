...
blog:
  ...
  cardDescription: Bluetape4k 소개 시리즈 Part 1. Application, Domain Capability, Data, Infrastructure, Foundation layer로 전체 구조를 짚어봅니다.
---

<figure class="bt4k-blog-hero">
  <img src="/assets/bluetape4k-ecosystem-hero.png" alt="bluetape4k backend 생태계를 연결된 building block으로 보여주는 소개용 일러스트" loading="eager" />
  <figcaption>생태계를 이해하려면 먼저 각 책임이 어떻게 연결되는지부터 봐야 한다.</figcaption>
</figure>

<p class="bt4k-post-meta">Part 1 · Ecosystem overview</p>

Bluetape4k는 Kotlin/JVM 기반 서비스를 만들 때 반복해서 필요한 기능을 모듈화한 라이브러리 생태계다.
애플리케이션 전체를 하나의 프레임워크로 감싸기보다, Spring Boot 4와 Ktor 3 서비스가 필요한 layer의
기능을 골라 붙일 수 있게 만든 쪽에 가깝다.

이 글은 전체 지도를 그리는 글이다. 각 저장소의 상세 architecture, API 예제, 적용 방식은 Part 2 이후에서
다룬다.
...
Bluetape4k를 처음 볼 때는 다섯 layer로 나누어 보면 이해하기 쉽다. 아래 표는 전체 그림을 잡기 위한
요약이고, 각 layer의 구체적인 모듈과 역할은 이어지는 섹션에서 따로 설명한다.
...
<figure class="bt4k-architecture">
  ...
  <figcaption>각 layer는 저장소 이름보다 해결하려는 문제 영역을 기준으로 묶었다.</figcaption>
</figure>

Application layer는 Spring Boot 4 또는 Ktor 3가 맡는다. Bluetape4k는 그 아래에서 domain, data,
infrastructure, foundation layer를 채우는 모듈을 제공한다.

## Application Layer

### Spring Boot 4

개발 목적: Spring Boot 기반 서비스에서 Bluetape4k의 data, cache, infra, testing 모듈을 자연스럽게
조립할 수 있게 한다.
...
개발 목적: Ktor 서비스에서도 cloud, coordination, graph 기능을 suspend API 중심으로 가져다 쓸 수
있게 한다.
...
개발 목적: 분산 환경에서 batch, scheduler, polling worker, migration gate 같은 작업이 중복 실행되지
않게 제어한다.
...
개발 목적: row-level CDC에만 기대지 않고, domain object 기준으로 변경 이력을 추적한다.
...
개발 목적: 일반적인 JVM 이미지 처리와 libvips 기반 대량 처리를 같은 생태계 안에서 선택할 수 있게
한다.
...
개발 목적: 한국어/일본어 중심의 tokenizer, language detection, keyword search 기능을 Kotlin 서비스에서
재사용할 수 있게 한다.
...
개발 목적: JPA 스타일보다 Kotlin DSL에 가까운 persistence model을 제공하고, JDBC와 R2DBC를 같은
repository pattern 안에서 다룬다.
...
개발 목적: graph database별 API 차이를 줄이고, 그래프 비중이 큰 도메인을 공통 abstraction으로 다룬다.
...
개발 목적: AWS Java SDK v2와 AWS Kotlin SDK를 Kotlin 서비스 코드의 실행 모델에 맞게 사용할 수 있게 한다.
...
개발 목적: 이벤트 기반 서비스와 비동기 연동에 필요한 messaging 기반을 제공한다.
...
개발 목적: local cache, distributed cache, Redis client integration을 서비스 계층에서 같은 패턴으로
다룰 수 있게 한다.
...
개발 목적: 운영 환경에 필요한 metrics, tracing, resilience, rate limiting 기능을 나머지 모듈과 함께
연결한다.
...
개발 목적: Kotlin coroutine과 Java Virtual Threads를 서비스 특성에 맞게 고를 수 있게 한다.
...
개발 목적: 반복되는 test setup을 줄이고, integration test를 빠르게 구성할 수 있게 한다.
...
개발 목적: 여러 저장소가 따로 발전해도 사용자가 호환 가능한 dependency set을 가져갈 수 있게 한다.
...
- 저장소 간 release cadence가 달라도 안정적인 조합을 제공

## How The Layers Work Together

실제 서비스에서는 Spring Boot 4 또는 Ktor 3가 application boundary가 된다. 그 아래에 Foundation, Data,
Infrastructure, Domain Capability layer를 필요한 만큼 조합한다.
...
보통은 먼저 Foundation layer에서 core, coroutine, testing, BOM을 가져오고, 이후 Data layer와
Infrastructure layer를 붙인다. 마지막으로 leader election, audit trail, image processing, text
processing 같은 Domain Capability를 서비스 요구에 맞게 선택한다.

Part 1에서 중요한 점은 Bluetape4k가 "하나의 거대한 framework"가 아니라는 것이다. 필요한 layer와
module을 작은 단위로 선택해, 이미 쓰고 있는 Spring Boot 4/Ktor 3 서비스에 점진적으로 얹는 생태계다.

다음 글부터는 각 layer를 더 작게 나누어, 대표 저장소의 architecture와 실제 사용 예제를 살펴본다.
