...
둘 중 하나가 항상 정답은 아니다. `scrimage`는 배포가 단순하고 native dependency가 없다. 반면 대량
thumbnail, resize, encode처럼 throughput이 중요한 경로에서는 `libvips`의 demand-driven pipeline이
훨씬 유리하다.
...
fallback으로 측정했기 때문에 codec 작업의 실제 texture, noise, chroma variation을 대표하기 어려웠다.
...
그래도 단색 fallback보다는 실제 photo pipeline의 의사결정에 훨씬 가까운 기준점이다.
...
이번 macOS arm64 run에서는 Java 21 JNI 값을 `N/A`로 표시했다. 이유는 Java 21이 느려서가 아니다.
...
각 variant는 resize와 encode를 모두 수행한다. 그 경로에서 `scrimage`와 `libvips`의 차이는 단일
숫자보다 더 크게 체감된다.

## 실무 적용 기준

간단한 이미지 처리나 native dependency를 피해야 하는 환경이라면 `images`의 scrimage path가 좋은
...
test를 `N/A`로 처리하고, Linux 또는 architecture-compatible host에서 별도 측정하는 편이 안전하다.
...
시작하고, throughput이 필요해지는 순간 `libvips` backend로 이동할 수 있게 같은 ecosystem 안에 두
경로를 제공하는 것이다.
