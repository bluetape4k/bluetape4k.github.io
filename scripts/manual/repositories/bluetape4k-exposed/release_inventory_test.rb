...
class ReleaseInventoryTest < Minitest::Test
  def test_filters_current_inventory_against_the_release_tree
    Dir.mktmpdir("release-inventory") do |root|
      ...
      rows = [
        { "gradlePath" => ":bluetape4k-exposed-druid", "sourceDir" => "exposed/druid" },
        { "gradlePath" => ":examples-ddd-spring-modulith-demo", "sourceDir" => "examples/ddd" },
        { "gradlePath" => ":examples-ktor-exposed-demo", "sourceDir" => "examples/ktor" },
      ]
      File.write(input, JSON.generate(rows))
      runner = lambda do |args|
        case args
        when ["rev-parse", "--verify", "refs/tags/1.11.0^{commit}"] then [("a" * 40) + "\n", true]
        when ["ls-tree", "-r", "--name-only", "a" * 40] then ["examples/ktor/build.gradle.kts\n", true]
        else ["", false]
        end
      end
      result = ManualDocs::ReleaseInventory.new(
        repository_root: root, tag: "1.11.0", expected_sha: "a" * 40,
        inventory_path: input, output_path: output, expected_count: 1, git_runner: runner,
      ).write
      assert_equal [":examples-ktor-exposed-demo"], result.map { |row| row.fetch("gradlePath") }
  ...
  def test_rejects_a_moved_release_tag
    with_inventory([{ "gradlePath" => ":core", "sourceDir" => "core" }]) do |root, input, output|
      ...
      assert_equal "release tag 1.11.0 resolves to #{'b' * 40}, expected #{'a' * 40}", error.message
    end
  end
