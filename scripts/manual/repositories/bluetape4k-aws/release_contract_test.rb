...
class ReleaseContractTest < Minitest::Test
  ...
  def test_reports_a_release_relative_link_absent_from_the_tag
    Dir.mktmpdir("release-contract") do |root|
      ...
      File.write(File.join(root, "docs/manual/en/modules/core.md"), "[Missing](../../../../aws-java/src/Missing.kt)\n")
      runner = lambda do |args|
        case args
        when ["rev-parse", "--verify", "refs/tags/0.4.0^{commit}"] then [("a" * 40) + "\n", true]
        when ["ls-tree", "-r", "--name-only", "a" * 40] then ["aws-java/src/Present.kt\n", true]
        else ["", false]
        end
      end
      result = ManualDocs::ReleaseContract.new(
      ...
      assert_equal ["docs/manual/en/modules/core.md:1: release path not found: aws-java/src/Missing.kt"], result.errors
  ...
  def test_validates_repository_relative_reference_links
    Dir.mktmpdir("release-contract") do |root|
      ...
      File.write(File.join(root, "docs/manual/en/modules/core.md"), <<~MARKDOWN)
        ...
        [source]: ../../../../aws-java/src/Missing.kt
      MARKDOWN
      runner = lambda do |args|
        case args
        when ["rev-parse", "--verify", "refs/tags/0.4.0^{commit}"] then [("a" * 40) + "\n", true]
  ...
  def test_validates_a_shallow_manuals_repository_relative_link
    Dir.mktmpdir("release-contract") do |root|
      ...
      File.write(File.join(root, "docs/manual/en/index.md"), "[Missing](../../../aws-java/src/Missing.kt)\n")
      runner = lambda do |args|
        case args
        when ["rev-parse", "--verify", "refs/tags/0.4.0^{commit}"] then [("a" * 40) + "\n", true]
        when ["ls-tree", "-r", "--name-only", "a" * 40] then ["aws-java/src/Present.kt\n", true]
      ...
      assert_equal ["docs/manual/en/index.md:1: release path not found: aws-java/src/Missing.kt"], result.errors
      assert_equal 1, result.checked_count
    end
  end
end
