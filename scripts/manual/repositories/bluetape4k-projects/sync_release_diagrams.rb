...
manual_root = File.join(root, "docs/manual/bluetape4k-projects")
...
if ARGV.any?
  ...
  inventory = File.expand_path(options.fetch("--inventory", File.join(manual_root, "release-diagrams.yaml")), root)
end
contract = ReleaseDiagrams::Contract.new(root: root, inventory_path: inventory, manual_root: manual_root)
