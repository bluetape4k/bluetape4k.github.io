...
module ReleaseDiagrams
  ...
  class Contract
    ...
    def initialize(root:, inventory_path:, require_diagram_provenance: true, manual_root: nil)
    ...
    def mirror_root
      @mirror_root ||= @manual_root.join("assets/readme-diagrams")
    end
    ...
    def resolved(path)
      value = path.to_s
      if value == "docs/manual"
        @manual_root
      elsif value.start_with?("docs/manual/")
        @manual_root.join(value.delete_prefix("docs/manual/")).cleanpath
      else
