...
import {
  awsSqsExtendedCompanion as companion,
...
const labels = {
  en: {
    ...
    light: 'Light',
    dark: 'Dark',
    choose: 'Choose a payload lifecycle',
    reset: 'Reset',
    play: 'Play',
    pause: 'Pause',
    next: 'Next',
    step: 'Step',
    sequence: 'Payload lifecycle sequence',
    ...
    scroll: 'Scroll horizontally to inspect the full payload lifecycle →',
    outcome: 'Outcome',
    pending: 'Not reached',
    branch: 'Inline, offload, and failure path',
  },
  ko: {
    catalog: '시각 자료',
    manual: 'AWS 매뉴얼',
    ...
    light: '밝게',
    dark: '어둡게',
    choose: 'Payload 수명주기를 선택하세요',
    reset: '처음으로',
    play: '재생',
    pause: '일시정지',
    next: '다음',
    step: '단계',
    sequence: 'Payload 수명주기 sequence',
    progress: '수명주기 진행률',
    active: '진행 중',
    waiting: '대기',
    complete: '완료',
    ...
    scroll: '전체 payload 수명주기는 가로로 스크롤해서 확인하세요 →',
    outcome: '결과',
    pending: '아직 도달하지 않음',
    branch: 'inline, offload, 실패 path',
  },
};
    ...
    scenarios: companion.scenarios.map((value) => ({
      ...
      failure: value.failure
        ? {
          ...
          text: pick(value.failure.text, locale),
          signal: pick(value.failure.signal, locale),
        }
        : null,
    })),
    stories: Object.fromEntries(companion.scenarios.map((value) => {
      const story = buildSqsExtendedStory(value.id);
      return [value.id, {
        ids: story.ids,
        failure: story.failure
          ? {
            at: story.failure.at,
            code: story.failure.code,
            text: pick(story.failure.text, locale),
            signal: pick(story.failure.signal, locale),
          }
          : null,
        terminal: story.terminal,
        s3Touched: story.s3Touched,
        handlerReached: story.handlerReached,
        sqsDeleted: story.sqsDeleted,
        cleanupRequired: story.cleanupRequired,
...
function sequenceRows(model, copy) {
  const positions = Object.fromEntries(model.participants.map((value, index) => [value.id, 10 + index * 20]));
  return model.steps.map((value, index) => {
    const from = positions[value.from];
    const to = positions[value.to];
    const self = from === to;
    const start = Math.min(from, to);
    const end = Math.max(from, to);
    const midpoint = self ? Math.min(from + 10, 90) : (from + to) / 2;
    const direction = self ? 'self' : to > from ? 'forward' : 'reverse';
    return `<li class="message-row tone-${value.tone}" data-step-index="${index}" data-from="${value.from}" data-to="${value.to}" data-direction="${direction}">
      ...
      <div class="message-track${self ? ' is-self' : ''}" style="--from:${from}%;--to:${to}%;--start:${start}%;--end:${end}%;--mid:${midpoint}%;--rotation:${self || to < from ? '180deg' : '0deg'}"><span class="message-origin" aria-hidden="true"></span><span class="message-line" aria-hidden="true"></span><span class="message-head" aria-hidden="true">➤</span><span class="message-label">${esc(value.event)}</span></div>
...
<body>
  ...
  <main class="page">
    <header class="hero"><div><p class="eyebrow">#423 · bluetape4k-aws · ${companion.version} · SQS payload lifecycle</p><h1>${esc(pick(companion.title, locale))}</h1><p>${esc(pick(companion.summary, locale))}</p></div><aside class="invariant"><strong>${esc(pick(companion.invariant, locale))}</strong><br><br><span>01 size gate → 02 S3 upload → 03 SQS send → 04 restore → 05 handler → 06 ack → 07 cleanup</span></aside></header>
    <div id="bt4k-issue-423" data-sqs-extended-ready="true" data-sequence="true" data-scenario="inline" data-step="0">
      ...
      <section class="ownership"><div class="ownership-card"><strong>${copy.trust}</strong><p>${esc(pick(companion.ownership.boundary, locale))}</p></div><span class="boundary">│ ack before cleanup │</span><div class="ownership-card"><strong>${copy.policy}</strong><p>${esc(pick(companion.caveats, locale))}</p></div></section>
      <p class="hint">${copy.keyboard}</p><noscript><p>${copy.nojs} <a href="${asset}.png">${copy.diagram}</a></p></noscript>
    </div>
    <footer class="provenance"><strong>${copy.source}</strong><br><a href="https://github.com/bluetape4k/bluetape4k.github.io/issues/423">${copy.issue} #423</a><a href="https://github.com/bluetape4k/bluetape4k-aws/issues/455">SQS Extended Client issue #455</a>${companion.sources.slice(0, 4).map((source) => `<a href="${source.url}">${esc(source.label)}</a>`).join('')}<span>${copy.sourceRevision}: ${esc(companion.sourceRevision)}</span><br><a href="${asset}.png">${copy.diagram}</a></footer>
  </main>
  <script>
    (() => {
      const root = document.getElementById('bt4k-issue-423');
      ...
      const render = () => {
        ...
        root.dataset.status = terminal && current.cleanupRequired ? 'CLEANUP_REQUIRED' : failed ? 'FAILED' : terminal ? 'COMPLETE' : 'IN_PROGRESS';
        root.dataset.workflowReady = 'true';
        root.querySelector('[data-scenario-summary]').textContent = selected.summary;
        root.querySelector('[data-scenario-outcome]').textContent = terminal ? selected.outcome : labels.pending;
        ...
        root.querySelector('[data-detail="signal"]').textContent = failed ? current.failure.signal : item.signal;
