...
const output = resolve(process.env.VISUAL_OUTPUT || '/tmp/issue423-browser');
...
try {
  for (const locale of ['en', 'ko']) {
    ...
    await page.goto(url);
    const explorer = page.locator('#bt4k-issue-423');
    ...
    for (const scenario of companion.scenarios) {
      ...
      const expected = buildSqsExtendedStory(scenario.id);
      for (let step = 1; step < expected.ids.length; step += 1) {
        await page.locator('[data-action="next"]').click();
      }
      assert.equal(await explorer.getAttribute('data-terminal'), 'true');
      const expectedStatus = expected.cleanupRequired ? 'CLEANUP_REQUIRED' : expected.failClosed ? 'FAILED' : 'COMPLETE';
      assert.equal(await explorer.getAttribute('data-status'), expectedStatus);
      ...
      const visibleSignal = await page.locator('[data-detail="signal"]').innerText();
      const expectedSignal = expected.failure
        ? expected.failure.signal[locale]
        : expected.steps.at(-1).signal[locale];
      assert.equal(visibleSignal, expectedSignal, `${locale}/${scenario.id} exposes the terminal state signal`);
      if (scenario.id === 'inline') {
        for (const index of [1, 7, 8]) assert.match(await page.locator(`[data-step-index="${index}"]`).getAttribute('class'), /is-muted/);
      }
      if (scenario.id === 'ack-failure' || scenario.id === 'caller-owned-lifecycle') {
        for (const index of [7, 8]) assert.match(await page.locator(`[data-step-index="${index}"]`).getAttribute('class'), /is-muted/);
      }
    ...
    await page.locator('[data-scenario-button="offload-success"]').click();
    ...
    await page.locator('[data-action="play"]').click();
    await page.locator('#bt4k-issue-423[data-terminal="true"]').waitFor({ timeout: 20_000 });
    assert.equal(await page.locator('[data-action="play"]').getAttribute('aria-pressed'), 'false');
