...
- 이미지 벤치마크 소스 링크 14개가 현재 저장소의 `benchmark/images-benchmark` 경로에 존재하는지 한·영문에서 확인했다.
- 차트 네 장의 SVG XML, 표시 수치, 한·영문 자산 쌍, 확대 보기 제목을 검증했다.
- 전체 크기 PNG로 다크 배경의 대비와 라벨 여백을 확인했다.
- `node --test tests/ecosystem/blog-diagram-locales.test.mjs tests/ecosystem/diagram-lightbox.test.mjs`가 21개 테스트를 통과했다.
- `npm run build`, `git diff --check`, 한·영문 네 페이지의 로컬 HTTP 200 응답을 확인했다.
