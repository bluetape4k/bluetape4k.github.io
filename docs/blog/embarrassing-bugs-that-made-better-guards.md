...
description: 잘못된 가정에서 시작한 bluetape4k-projects 버그 수정 다섯 가지와, 어떻게 찾고 고치고 다시 막았는지 정리한 글.
sidebar:
  order: -202605290925
blog:
  date: 2026-05-29T09:25:00+09:00
  image: /assets/embarrassing-bugs-hero.png
  imageAlt: 버그 리포트가 검증 코드와 보호 장치로 바뀌는 소개용 일러스트
...
<figure class="bt4k-blog-hero">
  <img src="/assets/embarrassing-bugs-hero.png" alt="버그 리포트가 검증 코드와 보호 장치로 바뀌는 소개용 일러스트" loading="eager" />
  <figcaption>민망한 실수로 시작한 수정도, 다음 실수를 조용히 막는 보호 장치로 남으면 충분히 쓸모 있다.</figcaption>
</figure>

<p class="bt4k-post-meta">2026-05-29 · bluetape4k projects bug-fix note</p>

`bluetape4k-projects`를 고치다 보면 아주 고급스러운 버그보다 더 아픈 버그가 나온다.
코드가 어려워서 생긴 문제가 아니라, 보고 나면 "아, 나는 왜 이걸 믿었지?" 싶은 문제들이다.

이 글은 그런 사례만 모았다. 멋진 최적화나 큰 설계 이야기가 아니다. 작은 착각이 issue가 되고,
Nightly가 깨지고, 결국 테스트와 release gate로 바뀐 이야기다.

<figure class="bt4k-architecture" data-diagram-title="부끄러운 버그 수정의 반복 구조">
  <img src="/assets/embarrassing-bug-fix-loop-01-ko.png" alt="실수, 신호, 수정, 검증, 보호 장치로 이어지는 부끄러운 버그 수정의 반복 구조를 설명한 다이어그램" loading="lazy" />
...
| 어떻게 찾았나 | 증상, 실패 로그, round-trip, Nightly 신호 |
| 어떻게 고쳤나 | 실제 수정 방향 |
| 평가는 | 검증 결과와 남은 해석 |
| 다음 장치는 | 다시 같은 실수를 하지 않기 위한 보호 장치 |

## 1. 빈 properties 값이 `null`이 아니라 `0`이 됐다


이건 작고 민망한 버그다. Java Properties에서 `maxWaitMillis=` 같은 값은 사람이 보면 "값이 비어 있다"다.
그런데 `JavaPropsMapper`는 Properties 값을 타입 없는 문자열로 읽고, nullable numeric Kotlin constructor
parameter에 넣는 과정에서 빈 문자열을 `0`으로 만들었다.
...
검증은 형식별로 했다. Jackson 2와 Jackson 3의 named datasource properties round-trip
test가 각 4개씩 통과했고, 전체 Jackson 2/3 test도 통과했다.

여기서 중요한 점은 "CSV, TOML, YAML도 비슷하게 고치자"가 아니었다. 그건 또 다른 실수다.
Properties에서 발견한 bug를 모든 text format에 전파하면, 다음에는 형식별 의미를 망가뜨릴 수 있다.

### 다음 장치

재발 방지 장치는 간단하다. 형식별 bug는 형식별 mapper에서만 고친다. 그리고 configuration round-trip
test를 둔다. `null`, empty string, `0`은 설정 파일에서는 서로 다른 값이다.
...
수정 내용 중 하나는 MyBatis dynamic-sql join validation test를 현재 `on` 기반 Kotlin DSL과
현재 exception type에 맞추는 것이었다. 또 다른 내용은 secured Elasticsearch singleton reuse를 꺼서
stale CI credentials 문제를 피하는 것이었다.

### 어떻게 찾았나

Nightly가 알려줬다. local targeted test만 보면 놓치기 쉬운 external-service slice가 full Nightly에서
실패했다. 여기서 바로 production code를 고치지 않은 것이 중요했다. 먼저 "이 테스트가 지금의 API와
환경을 제대로 재현하나?"를 봤다.
...
있다. Nightly가 실패했을 때 "아마 flaky"로 넘기지 않고, 테스트가 현재 현실을 검증하는지 확인한
것이 핵심이다.
...
readiness, image version을 의심하게 된다. 그런데 Bolt query path는 이미 성공했다. 멈춘 곳은
Neo4j Java Driver의 `Driver.close()`였다.
...
cleanup 보호 장치는 이 정도로 명시적이어야 했다. query가 성공했으면 검증은 끝난 것이고, driver cleanup이
...
바꾸면 caller가 취소한 작업이 application error처럼 보인다.
...
"false로 바꿀 수 있는 실패"처럼 눌러버렸다. 결과적으로 caller thread의 interrupt status가 사라졌다.
...
shutdown lifecycle issue를 보면서 interrupt path를 따로 봤다. timeout과 interruption은 둘 다
...
기존 timeout 동작은 유지됐다. 달라진 것은 interrupt 처리 의미다. regression test는 port-based와
in-process server variant 모두에서 interrupt status 보존을 확인했다.
...
| 실수 | 잘못 믿은 것 | 실제 보호 장치 |
|---|---|---|
| Empty numeric property | 빈 값과 `0`은 적당히 구분되겠지 | 형식별 round-trip test |
...
부끄러운 버그의 장점은, 고치고 나면 보호 장치가 선명해진다는 점이다. 어려운 설계 논쟁이 아니라
...
마지막으로 `checkDisabledTests` 같은 release gate가 중요하다. 부끄러운 실수를 개인 기억에만 맡기면
반복된다. 보고서, test, gate, lesson으로
...
다섯 사례의 근거는 다음 issue와 PR이다.

- [issue #491](https://github.com/bluetape4k/bluetape4k-projects/issues/491)
- [issue #595](https://github.com/bluetape4k/bluetape4k-projects/issues/595), [PR #598](https://github.com/bluetape4k/bluetape4k-projects/pull/598)
- [issue #602](https://github.com/bluetape4k/bluetape4k-projects/issues/602), [PR #604](https://github.com/bluetape4k/bluetape4k-projects/pull/604)
- [issue #654](https://github.com/bluetape4k/bluetape4k-projects/issues/654), [PR #670](https://github.com/bluetape4k/bluetape4k-projects/pull/670)
- [issue #656](https://github.com/bluetape4k/bluetape4k-projects/issues/656), [issue #657](https://github.com/bluetape4k/bluetape4k-projects/issues/657), [PR #669](https://github.com/bluetape4k/bluetape4k-projects/pull/669)
- [issue #497](https://github.com/bluetape4k/bluetape4k-projects/issues/497), [PR #567](https://github.com/bluetape4k/bluetape4k-projects/pull/567)

![버그 수정 loop 요약](/assets/embarrassing-bug-fix-loop-01.png)
