...
- [x] **Step 2: Run the series test and confirm the expected failure**
...
- [x] **Step 3: Add the Implementation 9 registry entry**
...
- [x] **Step 2: Pass the semantic audit before rendering**
...
- [x] **Step 3: Implement diagram 1 in the deterministic generator**
...
- [x] **Step 4: Render and audit one locale asset at a time**
...
- [x] **Step 5: Inspect both PNGs at original size**
...
- [x] **Step 1: Open and record two sequence references**
...
- [x] **Step 2: Define the sequence semantics**
...
6. an explicit **최종 상태 결정** frame leads to `DUPLICATE`, `WAITING_GAP`, quarantine/conflict, or appended revision.
...
- [x] **Step 3: Implement proper sequence geometry**
...
- [x] **Step 4: Render and audit each locale separately**
...
- [x] **Step 5: Inspect both full-size PNGs and repair before moving on**
...
- [x] **Step 6: Commit both verified diagram families**
...
- [x] **Step 1: Inspect nearby hero references**
...
- [x] **Step 2: Generate one text-free 16:9 hero with `image_gen`**
...
- [x] **Step 3: Compare the output at full size and in a contact sheet**
...
- [x] **Step 4: Commit the accepted hero**
...
- [x] **Step 1: Create the local article shell**
...
- [x] **Step 2: Draft the patient-led narrative in this order**
...
- [x] **Step 3: Embed both Korean diagrams**
...
- [x] **Step 4: Apply Korean technical-writing and terminology review**

Use natural Korean sentence order and expand explanations where the reader needs domain context. Preserve the shared glossary: `예약 서비스`, `스냅숏`, `방문 계획`, `방문 약속`, `대기 목록`, `최종 상태 결정`, and `아웃박스(outbox)`. Remove literal translation, vague subjects, generic conclusions, unnecessary “한/하나”, and unsupported implementation certainty.

- [x] **Step 5: Run the series test**

Expected: the only remaining failure is the missing English article.

- [x] **Step 6: Commit the Korean article**
...
- [x] **Step 2: Preserve the implementation boundaries**
...
- [x] **Step 3: Run the series regression test**
...
- [x] **Step 4: Commit bilingual parity**
...
- [x] **Step 1: Recheck the live Clinic source and tests**
...
- [x] **Step 2: Compare the articles with nearby series posts**
...
- [x] **Step 3: Run terminology and placeholder scans**
...
- [x] **Step 4: Regenerate and reinspect any changed diagrams**
...
- [x] **Step 3: Perform browser QA in both themes**
...
- [x] **Step 4: Capture fresh route evidence**
...
- [x] **Step 5: Run final repository checks and commit**
...
- [x] The Korean and English articles contain the same technical claims, sequence, tables, code, links, diagrams, and series position.
- [x] The approved dermatology example naturally explains exact choice count and component-version provenance without being presented as the literal test fixture.
- [x] `PurchaseCompletedHandler` and `VisitPlanningEventHandler` responsibilities are clearly separated.
- [x] No sentence or diagram implies that `PackageExecutionPlanner` creates visit candidates or confirmed appointments.
- [x] The hero is unique within the series and both diagram families pass semantic, geometry, arrowhead-color, endpoint, sequence-style, and full-size visual checks.
- [x] Sequence call labels never overlap their message lines, all bends are visibly rounded, and the terminal outcome decision is explicit.
- [x] The Astro build, series test, both locale routes, asset routes, themes, and diagram large-view interaction pass locally.
