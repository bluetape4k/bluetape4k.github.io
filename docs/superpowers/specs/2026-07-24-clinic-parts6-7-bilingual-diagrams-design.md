...
- 현재 영문 문구, 레이아웃, 색상, 연결선과 정보 구조는 유지한다.
...
한국어 문구가 기존 상자 폭을 넘으면 먼저 의미를 보존한 짧은 표현으로 다듬고, 그래도 필요한 경우 같은 상자 안에서 줄바꿈한다. 상자 크기와 전체 레이아웃 변경은 마지막 수단으로 제한한다.
...
- **한국어 문구가 상자 폭을 초과할 수 있다.** 의미를 보존한 축약, 줄바꿈, 최소한의 좌표 조정 순서로 해결한다.
...
- **현지화 과정에서 연결선이나 정보 구조가 바뀔 수 있다.** 영어 자산을 기준선으로 삼아 텍스트와 필요한 텍스트 배치만 변경한다.
