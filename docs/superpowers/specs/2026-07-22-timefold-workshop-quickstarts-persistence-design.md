...
6. **종료와 실패 경계:** Solver 실행 종료, 실행 가능 해 확보, 최적성 보장, 작업 예외를 서로 다른 결과로
   구분한다. 제한 시간 안에 찾은 가장 좋은 해답은 최적 해가 아닐 수 있으며, Hard 제약 위반이 남은 해답과
   실행 중 예외도 같은 `COMPLETED` 상태로 숨기지 않는다.
7. **영속화 경계:** 현재 Exposed 예제가 보장하는 Score 컬럼 round-trip과 보장하지 않는 전체 해답 저장을
   나눈다.
8. **애플리케이션 설계:** 문제 입력, Solver 작업, 최고 해답, 승인된 업무 상태를 한 테이블에 섞지 않는
   저장 모델을 제안한다.
9. **테스트와 리뷰:** Constraint Verifier, Score 컬럼 round-trip, job lifecycle, 재시작·동시 업데이트에서
   확인할 항목을 정리한다.
10. **선택 기준:** 데모를 실제 기능으로 옮길 때 필요한 체크리스트로 끝낸다.

### Solver 종료와 최적화 실패의 판정

글은 다음 결과를 구분한다.

- **실행 가능 해 없음:** Hard 제약 위반이 남은 가장 좋은 해답이다. 작업은 끝났지만 업무에 바로 반영할 수
  없으며, 위반 제약과 부족한 자원을 운영자에게 보여 줘야 한다.
- **실행 가능하지만 최적성 미확인:** 제한 시간, 개선 정체, 조기 종료로 탐색이 끝났고 실행 가능 해를
  얻었지만 전역 최적 해임을 보장할 수 없다. 업무별 최소 품질 기준을 통과한 경우에만 승인 후보로 둔다.
- **실행 실패:** 문제 조회, Score 계산, Solver 실행, 콜백 또는 저장 과정에서 예외가 발생했다. quickstart의
  `withExceptionHandler`처럼 원인을 남기되 재시도 가능 여부와 부분 저장 정리 정책은 애플리케이션이 정한다.
- **취소:** 운영자나 시스템이 실행을 중단했다. 마지막 후보가 있더라도 승인 가능 여부를 명시적으로 판정한다.

운영 판정은 `SolverStatus`만 보지 않는다. 작업 종료 여부, Score의 실행 가능성, 업무별 최소 품질 기준,
현재 데이터 버전과의 충돌 여부를 차례로 확인한다. 이 범위는 한글과 영문 글에 같은 표와 설명으로 반영하고,
현재 다이어그램은 변경하지 않는다.
...
실행 가능성·최적성·예외·취소의 결과 판정, Exposed Score 저장 범위, 작업·후보 해답 저장소,
승인·업무 반영 책임을 설명한다. 코드, 수치, 표, 소스 링크와 섹션 순서는 두 언어에서 대응되어야 한다.
