...
The 1.0.0 DDD example deliberately demonstrates order, not atomicity: application persistence, JaVers commit, event publication, then consumer-side Redis projection. The [DDD/CQRS guide](../guides/ddd-and-cqrs.md) explains where an outbox and idempotent consumer belong in production.
