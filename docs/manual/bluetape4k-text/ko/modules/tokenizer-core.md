...
HTTP나 메시지 요청 모델, 공통 사전 도구, 자체 프로세서가 필요하다면 core를 직접 사용한다. 실제 형태소 분석이 목적이라면 [한국어](tokenizer-korean.md) 또는 [일본어](tokenizer-japanese.md) 모듈을 선택한다.
