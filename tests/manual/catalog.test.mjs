...
test('validates and normalizes a version catalog without mutating its input', () => {
  ...
  const result = validateVersionCatalog(catalog);
  ...
  assert.throws(() => validateVersionCatalog({ ...catalog, repository: 'evil/repo' }), /REPOSITORY_UNSUPPORTED/);
  assert.throws(() => validateVersionCatalog({ ...catalog, versions: [...catalog.versions, catalog.versions[1]] }), /CATALOG_DUPLICATE_MINOR/);
  assert.throws(() => validateVersionCatalog({ ...catalog, versions: [...catalog.versions].reverse() }), /CATALOG_UNSORTED/);
...
test('canonicalizes nested index document identifiers and selector routes', () => {
  ...
  const normalized = validateVersionCatalog(nested);
  ...
  }), { kind: 'document', href: '/ko/manual/bluetape4k-projects/1.11/guide/' });
});
...
test('merges a new version immutably and rejects duplicate document identifiers', () => {
  ...
  const merged = mergeVersionCatalog(previous, catalog.versions[1]);
...
test('builds cumulative latest redirects and rejects chains and loops', () => {
  ...
  const result = buildRedirectCatalog({ previous, latestEntry: catalog.versions[1] });
...
test('generates natural localized unavailable Markdown with a preserved return route', () => {
  const ko = buildUnavailablePage({ locale: 'ko', targetMinor: '1.10', sourceMinor: '1.11', documentId: 'modules/new' });
  ...
  const en = buildUnavailablePage({ locale: 'en', targetMinor: '1.10', sourceMinor: '1.11', documentId: 'modules/new' });
